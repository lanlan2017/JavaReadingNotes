...
<resultMap
    id="studentMap"
    type="com.blue.po.Student">
    <!-- 把当前查询结果集的 id 字段的值，作为Student对象的 id 成员变量的值 -->
    <id
        column="id"
        property="id" />
    <!-- 把当前查询结果集的 name 字段的值，作为Student对象的 name 成员变量的值 -->
    <result
        column="name"
        property="name" />
    <!-- 把当前查询结果集的 sex 字段的值，作为Student对象的 sex 成员变量的值 -->
    <result
        column="sex"
        property="sex" />
    <result
        column="age"
        property="age" />
    <!-- 把当前结果集的clazz_id字段的值，作为子查询的参数 -->
    <!-- select ：表示要执行的子查询语句的位置 -->
    <!-- property：Student对象中接收子查询结果集的成员变量 -->
    <!-- javaType：接收结果集的成员变量的类型 -->
    <association
        column="clazz_id"
        select="selectClassByID"
        property="class1"
        javaType="com.blue.po.Class" />
</resultMap>
<select
    id="selectClassByID"
    resultType="com.blue.po.Class">
    select * from
    tb_clazz where id=#{id}
</select>
```
...
<resultMap
    id="classMap"
    type="com.blue.po.Class">
    <!-- 当前查询结果集的id字段值，作为Class对象Id成员变量的值 -->
    <id
        column="id"
        property="id" />
    <!-- 当前查询结果集的code字段值，作为Class对象code成员变量的值 -->
    <result
        column="code"
        property="code" />

    <!-- column 把当前查询的id字段作为 子查询的参数 -->
    <!-- ofType 子查询的参数的类型 -->
    <!-- select 子查询的位置 -->
    <!-- property Class对象中接收子查询结果的成员变量 -->
    <!-- javaType 接收子查询结果集的成员变量的类型 -->
    <collection
        column="id"
        ofType="com.blue.po.Class"
        select="selectStudentByClassID"
        property="students"
        javaType="ArrayList"></collection>
</resultMap>
<select
    id="selectStudentByClassID"
    resultType="com.blue.po.Student">
...
- `ofType`。（子查询输入参数类型）表示集合当中的类型,本示例中是`Student`类型。
