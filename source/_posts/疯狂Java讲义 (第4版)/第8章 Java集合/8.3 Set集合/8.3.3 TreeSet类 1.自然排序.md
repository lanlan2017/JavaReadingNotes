...
- [8.3.3 TreeSet类](/JavaReadingNotes/cd1e976c/#8-3-3-TreeSet类)
    - [TreeSet是自动排序的集合](/JavaReadingNotes/cd1e976c/#TreeSet是自动排序的集合)
    - [TreeSet额外方法](/JavaReadingNotes/cd1e976c/#TreeSet额外方法)
    - [小结](/JavaReadingNotes/cd1e976c/#小结)
    - [实例](/JavaReadingNotes/cd1e976c/#实例)
- [1.自然排序](/JavaReadingNotes/cd1e976c/#1-自然排序)
    - [实现了Comparable接口的常见类](/JavaReadingNotes/cd1e976c/#实现了Comparable接口的常见类)
    - [TreeSet中的对象必须实现Comparable接口](/JavaReadingNotes/cd1e976c/#TreeSet中的对象必须实现Comparable接口)
    - [TreeSet中的第一个元素可以不实现Comparable接口](/JavaReadingNotes/cd1e976c/#TreeSet中的第一个元素可以不实现Comparable接口)
    - [TreeSet中应该存放用一个类的对象](/JavaReadingNotes/cd1e976c/#TreeSet中应该存放用一个类的对象)
    - [总结 TreeSet只能添加同一种类的对象](/JavaReadingNotes/cd1e976c/#总结-TreeSet只能添加同一种类的对象)
    - [相等的元素不会添加到TreeSet中](/JavaReadingNotes/cd1e976c/#相等的元素不会添加到TreeSet中)
    - [compareTo返回0则TreeSet就认为这两个对象相等](/JavaReadingNotes/cd1e976c/#compareTo返回0则TreeSet就认为这两个对象相等)
    - [TreeSet中元素的规则](/JavaReadingNotes/cd1e976c/#TreeSet中元素的规则)
        - [要确保equals方法返回true时compareTo返回0](/JavaReadingNotes/cd1e976c/#要确保equals方法返回true时compareTo返回0)
        - [equals方法返回false时compareTo返回0引起的问题](/JavaReadingNotes/cd1e976c/#equals方法返回false时compareTo返回0引起的问题)
    - [TreeSet只会在添加元素时排序](/JavaReadingNotes/cd1e976c/#TreeSet只会在添加元素时排序)
    - [不要修改TreeSet中对象的关键实例变量值](/JavaReadingNotes/cd1e976c/#不要修改TreeSet中对象的关键实例变量值)
